...
# === MongoDB Setup ===
try:
    client = MongoClient("mongodb://mongodb:27017/")  # docker-compose service name
    db = client["voiceflappy"]
    collection = db["predictions"]
    print("✅ Connected to MongoDB.")
except Exception as e:
    print(f"❌ MongoDB connection failed: {e}")
    collection = None  # So we don't crash later
...
def get_voice_command(chunk, threshold=DETECTION_THRESHOLD):
    ...
    if best_prob >= threshold:
        ...
        return label, best_prob, volume
    else:
        print("🕳️ No confident prediction.")
        return None
    

def log_prediction_to_db(command, confidence, volume, score):
    if collection is None:
        print("⚠️ Skipping DB logging (no DB connection)")
        return
    try:
...
def handle_voice_command(self, cmd):
    ...
    log_prediction_to_db(
        command=cmd,
        confidence=self.last_confidence,   # You'll store this temporarily below
        volume=self.last_volume,           # same
        score=self.score
    )
    ...
    def update(self):
        ...
        if self.frame_counter % PREDICTION_INTERVAL == 0 and not audio_queue.empty():
            ...
            result = get_voice_command(chunk)
            if isinstance(result, str):  # Only a label string was returned
                cmd = result
                self.last_confidence = None
                self.last_volume = None
            elif result:  # A tuple (label, confidence, volume)
                cmd, conf, vol = result
