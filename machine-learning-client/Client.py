...
class VoiceCommandClient:
    ...
    def load_model(self):
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"Standard model loading failed: {e}, trying with custom options")
                ...
                try:
                    ...
                    logger.warning(f"Custom options loading failed: {inner_e}, trying tf.keras.models.load_model")
    ...
    def predict(self, audio_data):
        ...
        try:
            ...
            try:
                predictions = self.model.predict(features, verbose=0)  # Set verbose=0 to reduce log noise
            except Exception as e:
            ...
            if predicted_class == "down" and "up" in top_classes:
                ...
                if abs(confidence - up_confidence) < 0.2:  # They're close
                    logger.info(f"🔄 Up/Down confusion detected! down:{confidence:.4f} vs up:{up_confidence:.4f}")
                    # Bias toward "up" since it seems to be under-recognized 
                    if up_confidence > confidence * 0.8:  # Add significant bias toward "up"
                        predicted_class = "up"
                        confidence = up_confidence
                        logger.info(f"🔄 Resolving up/down confusion: choosing 'up' command")
            
            # Apply more sophisticated decision logic
            
            # 1. If the top prediction is very confident, use it
            if confidence > 0.7:
                logger.info(f"High confidence prediction: {predicted_class} ({confidence:.4f})")
                # Just a sanity check for the known commands
                if predicted_class not in ["up", "down", "go", "stop", "background"]:
                    logger.warning(f"Unknown command predicted with high confidence: {predicted_class}")
            ...
            # 2. For game control commands (up/down), use a lower threshold
                ...
                logger.info(f"Game control command accepted: {predicted_class} ({confidence:.4f})")
                
            # 3. For other commands, use a medium threshold
            elif predicted_class in ["go", "stop"] and confidence > 0.4:
                logger.info(f"Game command accepted: {predicted_class} ({confidence:.4f})")
                
            # 4. If top two predictions are close, prefer game commands over background
            elif len(top_confidences) >= 2 and (top_confidences[0] - top_confidences[1] < 0.2):
                # If the difference is small and second prediction is a game command, use it instead
                if top_classes[0] == "background" and top_classes[1] in ["up", "down", "go", "stop"]:
                    predicted_class = top_classes[1]
                    confidence = top_confidences[1]
                    logger.info(f"Preferring game command over background: {predicted_class} ({confidence:.4f})")
                # Similarly, prefer up/down over other commands when close
                elif top_classes[1] in ["up", "down"] and top_classes[0] not in ["up", "down"] and top_confidences[1] > 0.35:
                    predicted_class = top_classes[1]
                    confidence = top_confidences[1]
                    logger.info(f"Preferring directional command: {predicted_class} ({confidence:.4f})")
                    
            # 5. If confidence is too low, return background
            elif confidence < 0.3:
                logger.info(f"Low confidence ({confidence:.4f}), treating as background")
                ...
                if self.up_down_confusion_count >= 2 and predicted_class == "down":
                    ...
                    for idx, cls in enumerate(top_classes):
                        if cls == "up" and top_confidences[idx] > 0.3:
                            ...
                            logger.info("🔄 Applied adaptive correction: 'down' → 'up' based on pattern")
                            break
...
def predict_api():
    ...
    try:
        ...
        try:
            ...
            # Log detailed info
            logger.info(f"API prediction result: {predicted_class} with confidence {confidence:.4f}")
