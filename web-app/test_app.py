...
class TestVoiceCommandGameApp(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.scores_patch = patch.object(app, 'scores_collection', self.mock_scores_collection)
        self.commands_patch = patch.object(app, 'commands_collection', self.mock_commands_collection)
    ...
    def test_scores_route_with_scores(self):
        ...
        self.mock_scores_collection.find.return_value.sort.return_value.limit.return_value = mock_scores
    ...
    def test_get_commands_with_data(self):
        ...
        self.mock_commands_collection.find.return_value.sort.return_value.limit.return_value = mock_commands
    ...
    def test_handle_audio_valid_command(self):
        ...
        with patch('tempfile.NamedTemporaryFile') as mock_tempfile, \
             ...
             patch('app.predict_command') as mock_predict, \
            ...
            with patch.dict('sys.modules', {'pydub': MagicMock()}):
                ...
                with patch('pydub.AudioSegment', MagicMock()) as mock_audio_segment_class:
                    ...
                    socket_client.emit('audio', 'data:audio/webm;base64,SGVsbG8gV29ybGQ=')
    ...
    def test_handle_audio_error(self):
        ...
        with patch('tempfile.NamedTemporaryFile') as mock_tempfile, \
            ...
            with patch.dict('sys.modules', {'pydub': MagicMock()}):
                ...
                with patch('pydub.AudioSegment', MagicMock()) as mock_audio_segment_class:
                    mock_audio_segment_class.from_file.side_effect = Exception("Audio processing error")
