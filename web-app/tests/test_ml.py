...
def test_predict_command():
    ...
    try:
        ...
        with patch("librosa.load") as mock_load, patch(
        ...
        ) as mock_melspec, patch("librosa.power_to_db") as mock_power_to_db, patch(
            "app.model"
        ) as mock_model, patch(
            "app.label_encoder"
        ) as mock_label_encoder:
            ...
            assert (
                result == "background"
            )  # Should return "background" for low confidence
...
def test_load_model_func():
    ...
    with patch("tensorflow.keras.models.load_model") as mock_tf_load, patch(
        "joblib.load"
    ) as mock_joblib_load, patch("os.path.exists") as mock_exists, patch(
        ...
        with patch("time.sleep") as mock_sleep:
            ...
            )  # Should have tried to sleep twice (3 attempts - 1)
...
def test_predict_command_detailed():
    ...
    try:
        ...
        with patch("librosa.load") as mock_load, patch(
            ...
            mock_melspec.return_value = np.zeros(
                (128, 88)
            )  # Double-size mel spectrogram
            mock_power_to_db.return_value = np.zeros((128, 88))  # Double-size log mel

            mock_model.predict.return_value = np.array(
                [[0.1, 0.0, 0.9]]
            )  # High confidence for "down"
