...
def test_audio_no_model(socketio_client):
    ...
    test_data = "data:audio/webm;base64," + base64.b64encode(b"test audio data").decode(
        "utf-8"
    )
...
def test_audio_processing(
    mock_predict, mock_encoder, mock_model, mock_temp, socketio_client, mock_mongodb
):
    ...
    with patch("pydub.AudioSegment.from_file") as mock_from_file:
        ...
        with patch("app.commands_collection", mock_commands):
            ...
            # Send dummy audio data
            test_data = "data:audio/webm;base64," + base64.b64encode(
                b"test audio data"
            ).decode("utf-8")
            socketio_client.emit("audio", test_data)
...
def test_socketio_handle_audio_detailed(mock_predict, socketio_client, mock_mongodb):
    ...
    try:
        ...
        ) as mock_audio_segment, patch("os.remove") as mock_remove, patch(
            "app.commands_collection", mock_commands
        ), patch(
            "app.model", MagicMock()
        ), patch(
            "app.label_encoder", MagicMock()
        ):
